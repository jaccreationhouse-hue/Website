    ...
    <Container className="animated-fade">
      ...
      <Card>
        <Form onSubmit={onSubmit}>
          <Input value={title} onChange={(e) => { setTitle(e.target.value); if (!id) setSlug(slugify(e.target.value)); }} placeholder="Title" required />
          <Input value={slug} onChange={(e) => setSlug(slugify(e.target.value))} placeholder="slug" required />
          <Input value={value} onChange={(e) => setValue(e.target.value)} placeholder="2,300+" required />
